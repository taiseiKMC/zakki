...
1. $p_0$ と $p_1$ を結ぶ直線と楕円曲線の($p_0, p_1$ とは異なる)交点のx軸対称の点 (y 座標をマイナスにしたもの. この点も P 上にある)
2. $p_0$ と $p_1$ が等しい場合, $p_0$ における楕円曲線の接線と, 楕円曲線の($p_0$ とは異なる)交点のx軸対称の点
...
1 のケースだと $l = \frac{y_2 - y_1}{x_2 - x_1}$ ですね.
...
他にも NIST による標準化が知られていますが, その P-256 は secp256**r**1 と同じです.
...
* 現実的にまず起きませんが, ECDSA の仕様として r または s が 0 の場合は k を選び直して署名を再生成することになっています.
...
[^integer]: DER の Integer は先頭の bit が1だと負の数扱いとなります. s は low-s 形へ正規化することで 32bytes にできるが, r は 32 bytesだったり33 bytes だったりして DER 形式のデータ長に微妙に差が発生します.
